...
public class TaxReverseWorkItemWorker extends AbstractWorkItemWorker
{
	private static final Logger LOG = LoggerFactory.getLogger(TaxReverseWorkItemWorker.class);
	...
	protected void executeInternal(final Map<String, Object> parameters)
	{
		...
		LOG.debug("Starting Tax Reverse of return id: {}", returnId);
		...
		try
		{
			...
			cisService.reverseTax(aReturn);
		}
		catch (final HybrisSystemException e)
		{
			LOG.error("Reverse tax failed !", e);
			success = false;
		}

		parameters.put(CoreWorkflowConstants.KEY_OUTCOME_NAME, success);

		LOG.debug("Done Tax Reverse of return id: {}. Outcome: {}", returnId, Boolean.toString(success));
	}
