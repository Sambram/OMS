...
/**
 * WebResource exposing {@link UiOrderFacade} http://localhost:8080/oms-rest-webapp/webresources/uiorders/{orderId}.
 */
@Component
@Path("/uiorders/{orderId}")
...
public class UIOrderResource
{
	...
	/**
	 * Get one {@link UIOrder} by unique order id.
	 * 
	 * @param orderId the order id which is used to fetch the order object
	 * @return A response with an UIOrder object embedded in it.
	 */
	@GET
	@TypeHint(UIOrder.class)
	public Response getUIOrderByOrderId(@PathParam("orderId") final String orderId)
	{
		final Response.ResponseBuilder response = Response.ok();
		response.entity(this.uiOrderFacade.getUIOrderByOrderId(orderId));
