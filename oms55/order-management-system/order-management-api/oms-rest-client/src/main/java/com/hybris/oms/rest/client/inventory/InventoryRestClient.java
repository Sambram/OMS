...
public class InventoryRestClient extends DefaultRestClient implements InventoryFacade
{
	...
	public Pageable<ItemLocation> findItemLocationsByQuery(final ItemLocationsQueryObject queryObject)
	{
		...
		try
		{
			final RestResponse<?> response = call.get(ItemLocationList.class);
			final ItemLocationList itemLocationList = (ItemLocationList) response.result();
			final List<ItemLocation> itemLocations = itemLocationList.getItemLocations();
