...
/**
 * Implementation of {@link AtsChangeTypecodeHandler} for typecode represented by {@link OrderLineQuantityData}.
 */
public class OrderLineQuantityAtsChangeTypecodeHandler extends AtsChangeTypecodeHandler<OrderLineQuantityData>
{

	@Override
	public String getSku(final Map<String, Object> values) throws AtsChangeListenerException
	{
		final HybrisId id = (HybrisId) values.get(OrderLineQuantityData.ORDERLINE.name());
		try
		{
			final OrderLineData orderLine = getPersistenceManager().get(id);
			return orderLine.getSkuId();
		}
	...
	{
		return (String) values.get(OrderLineQuantityData.STOCKROOMLOCATIONID.name());
	}
